...
- `docker compose -f docker-compose.yml -f docker-compose.override.yml -f docker-compose.vg-dev.yml config` passed.
- `docker-compose.yml` matches upstream `v2026.1.1` exactly.
- Nginx Mocha CSP definition checks ran and passed before the live nginx
  connection-dependent tests started.
...
- Live nginx/server integration tests were not run because the compose stack was
  not running; `test:nginx:mocha` stopped at the HTTP/HTTPS connection hook with
  `EPERM`.

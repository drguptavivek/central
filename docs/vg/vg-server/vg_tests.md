...
- Integration:
  - `test/integration/api/vg-app-user-auth.js`: 72
...
| IP rate limiting settings (configurable via API) | `test/integration/api/vg-app-user-auth.js` | ✅ Pass | System/project settings for IP max failures, window, lock duration; validation; project overrides | `NODE_CONFIG_ENV=test BCRYPT=insecure npx mocha test/integration/api/vg-app-user-auth.js --grep "IP rate limiting settings"` |
| Settings endpoints security (RBAC) | `test/integration/api/vg-app-user-auth.js` | ✅ Pass | App users blocked from system/project settings; project permissions enforced; cross-project access blocked; admin access allowed | `NODE_CONFIG_ENV=test BCRYPT=insecure npx mocha test/integration/api/vg-app-user-auth.js --grep "Settings endpoints security"` |
...
- ✅ `NODE_CONFIG_ENV=test BCRYPT=insecure npx mocha test/integration/api/vg-app-user-auth.js --grep "IP rate limiting settings"` (settings API tests)
- ✅ `NODE_CONFIG_ENV=test BCRYPT=insecure npx mocha test/integration/api/vg-app-user-auth.js --grep "Settings endpoints security"` (security tests)

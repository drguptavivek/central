...
- **scripts/start-stack.sh** - Validate and start stack
- **garage/setup-garage.py** - Garage S3 setup (auto-called)
- **scripts/generate-s3-conf.sh** - Generate nginx S3 config
...
- Startup validation script (scripts/start-stack.sh)
...
│   ├── start-stack.sh        # Stack startup with validation
│   └── generate-s3-conf.sh   # Generate S3 nginx config
│
...
│
├── docker-compose.yml         # Main service definitions (includes Garage)
├── .env.template             # Environment variable template
└── garage/                     # Garage S3 configuration
    ├── setup-garage.py       # Auto-setup script
    └── garage.toml           # Generated by setup script
```
